...
from utils import get_type_code, load_and_import_file, take_window_of_center
...
class CardsCreater:
    def __init__(self, token, window):
        ...
        self.names_all_decks = {}
        self.names_of_decks = [None]
        all_decks = requests.get(
            url="http://127.0.0.1:8000/api/v1/decks/",
            headers={
                'Authorization': f'Token {self.token}'
            }
        ).json()
        if all_decks:
            self.names_all_decks = {style['name']: style['id'] for style in all_decks}
            self.names_of_decks = [deck for deck in self.names_all_decks.keys()]
        all_card_styles = requests.get(
            url="http://127.0.0.1:8000/api/v1/styles/",
            headers={
                'Authorization': f'Token {self.token}'
            }
        ).json()
        self.names_all_card_styles = {style['name']: style['id'] for style in all_card_styles}
        self.names_of_styles = [style for style in self.names_all_card_styles.keys()]
    ...
    def create(self):
        ...
        if deck_id:
            style_id = self.names_all_card_styles.get(self.card_style.get())
